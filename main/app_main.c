...
#define BROKER_URL "mqtt://dev.idals.com.br"
...
#define LOG(STR) puts(COLOR_CYAN STR "\n" COLOR_RESET);
...
static bno055_9dof_t bno055_data = {0};
...
static void mqtt_send_task(void *args) {
  ...
  while(1) {
    ...
    // char *buff = features_to_json(&feats);
    printf(COLOR_CYAN"%s\n"COLOR_RESET, buff);
...
char *features_to_json(Features *f) {
  char buff[255] = {0};
  sprintf(buff, "{ "
    "\"origin\": \"esp\", "
    "\"accel_x\": %f, "
